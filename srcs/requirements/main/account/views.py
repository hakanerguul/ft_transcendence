...
def account_view(request, *args, **kwargs):
    ...
    if request.user.is_authenticated:
        ...
        if not is_blocked and Friend.objects.get(user=account).is_blocked(request.user):
            return HttpResponseForbidden(_("You are blocked by this {} user.").format(account))
    ...
    context['online'] = online(account.id)
    context['last_seen'] = cache.get('seen_%s' % account.id)
...
def edit_account_view(request, *args, **kwargs):
    ...
    if account.pk != request.user.pk:
        return HttpResponse(_("You cannot edit this account"))
    context = {}
    if request.POST:
...
def user_status(request):
    if request.method == 'POST':
        ...
        if user_id:
            return JsonResponse({'status': online(user_id), 'last_seen': cache.get('seen_%s' % user_id)})
        else:
...
def home_view(request):
    return render(request, 'html/home.html', {'users': User.objects.all().order_by('username')})
